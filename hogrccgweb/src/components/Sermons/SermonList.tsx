...
const SermonList = () => {
    ...
    // Filter sermons by selected tag
    const filteredSermons = selectedTag
      ? SermonData.filter((sermon) => sermon.tags.includes(selectedTag))
      : SermonData;
    ...
    return (
        ...
        <Box sx={{backgroundColor: "white"}}>
            {/* Filter Section */}
            <Stack direction="row" spacing={2} justifyContent="center" mb={4} flexWrap="wrap" sx={{ gap: { xs: 2, sm: 2 }} }>
  {["Faith", "Prayer", "Hope", "Life Challenges", "Trust"].map((tag) => (
    ...
    />
     ...
     {/* Sermons Section */}
     <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}  sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', paddingRight: {md:"50px", xs:"20px"}, paddingLeft: {md:"50px", xs:"20px"} }}>
    ...
    <Grid size={{ xs: 12, sm: 6, md: 4 }} key={sermon.id}>
      <Card>
        ...
        <CardContent>
          ...
          <Typography variant="body2" sx={{ marginTop: '10px', color: '#333' }}>
            {sermon.description}
          </Typography>
        </CardContent>
        <Box sx={{ padding: '10px', textAlign: "end" }}>
          <Button
            variant="contained"
            color="primary"
            onClick={() => handleWatchClick(sermon.videoUrl)}
            sx={{border: "1px solid #0045B5", backgroundColor: "transparent", color: "black", transition: 'box-shadow backgroundColor: #0045b5 0.3s ease', '&:hover':{backgroundColor:"#0045B5", color: "white"}}}
          >
            Watch Now
          </Button>
        </Box>
      ...
      <div className="bg-white shadow-lg flex justify-center items-center flex-col rounded-[5px] pt-20 pb-20 md:px-[50px] px-[20px]">
          <h2 className="font-headingFont text-hogblack text-center text-[30px] mb-2 font-semibold"> Never Miss a Sermon!</h2>
          <p className="font-paragraphFont text-hogblack text-center text-[20px] mb-3">Subscribe to our YouTube channel to stay updated with the latest messages and teachings.</p>
          <a
